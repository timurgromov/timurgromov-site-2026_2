...
- **Username** — логин GitHub; **Password** при HTTPS — строка **PAT** (не пароль от сайта).
- Для пуша кода и ветки `main` достаточно **`repo`**. Чтобы пушить **`.github/workflows/*.yml`**, у того же PAT дополнительно нужен scope **`workflow`** (classic) или аналог в fine-grained.
- На macOS токен обычно сохраняют в **Связку ключей** после первого успешного `git push`.

Пуш в **`gh-pages` с локальной машины не обязателен** — этим занимается Actions через `GITHUB_TOKEN`.

### 4.1. Срок действия PAT (90 дней)

Если при создании classic-токена выбран срок **90 days**, дата окончания — **та, что GitHub показывает на экране создания** (это **+90 дней от момента нажатия Generate**, не от «сегодня» в каком-то другом календаре). После истечения нужно выпустить новый токен и снова ввести его при `git push` (или обновить запись в Связке ключей для `github.com`).

### 4.2. Безопасность — никогда не класть токен в репозиторий

- **Не** вставляй PAT в `docs/`, README, комментарии, `.env`, коммиты и **не** присылай в чаты — строка `ghp_…` даёт доступ к репозиторию.
- Храни только в **Связке ключей** / менеджере паролей. Если токен засветился где угодно (чат, скрин, публичный репо) — сразу **Revoke** на https://github.com/settings/tokens и создай **новый**.
