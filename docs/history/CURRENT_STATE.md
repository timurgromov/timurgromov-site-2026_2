...
Сайт свадебного ведущего Тимура Громова на Astro поверх Tilda export. Текущий этап: production-входная воронка сайта подключена к EventBudjet CRM, tripwire-сценарий переведён на квалификационную форму перед переходом в Telegram.
...
- В CTA-блоке сценария есть открытая inline-форма с теми же вопросами, чтобы пользователь мог получить материал без дополнительного pop-up.
- Tripwire-форма спрашивает `Кто вы?`, `Когда свадьба?`, `Где будет свадьба?` через компактные раскрывающиеся поля и необязательный свободный комментарий `Что уже известно?`.
