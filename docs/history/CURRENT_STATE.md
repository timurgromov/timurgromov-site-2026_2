...
Сайт свадебного ведущего Тимура Громова на Astro поверх Tilda export. Текущий этап: production-входная воронка сайта подключена к EventBudjet CRM, продолжаются UX-улучшения popup-формы.
...
- Telegram deep links зафиксированы: `site_plan` для получения плана и `site_meeting` для встречи.
...
- Production backend уже принимает site consultation requests с `timurgromov.ru`, создаёт карточки в `Все заявки` и отправляет мгновенные Telegram-уведомления в канал CRM-заявок.
