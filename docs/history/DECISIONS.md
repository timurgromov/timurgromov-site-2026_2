...
Decision:
Для субъективных визуальных задач недостаточно доказать только то, что код ушел в `main` и опубликовался. Задача считается завершенной только если есть либо визуальное подтверждение live-страницы в нужном viewport, либо точное before/after описание с предупреждением, что изменение может быть subtle.

Why:
Marker-check и факт deploy доказывают публикацию, но не доказывают, что владелец увидит заметный визуальный эффект. На тонких UI-правках агент может технически сказать правду про commit/push/deploy и одновременно ошибочно закрыть задачу по качеству результата.

Do:

- для задач `сделай как раньше`, `уменьши`, `аккуратнее`, `слишком громоздко`, `не похоже на старый дизайн` делать visual proof или numeric disclosure
...
- не считать live marker-check достаточным доказательством для дизайн-задачи
