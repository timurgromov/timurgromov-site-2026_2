...
## DEC-2026-06-16-COPY-BAN-SPOKOYNO

Status: active
Area: copywriting, ux, brand
Decision date: 2026-06-16
Evidence: прямое указание пользователя после повторного появления формулировок уровня `спокойно познакомимся`
Commits: pending
Supersedes: none

Decision:
В пользовательском тексте сайта запрещено использовать слово `спокойно` и однокоренные варианты: `спокойный`, `спокойная`, `спокойное`, `спокойные`, `спокойнее`, `спокойного`, `спокойной`.

Why:
Пользователь считает эту формулировку неестественной для русского языка и слабой для бренда. Она звучит generic и размывает прямой деловой тон сайта.

Do:

- писать прямо: `познакомимся по делу`, `разберём формат`, `понятный план`, `без давления`, `без лишнего напряжения`
- перед изменениями публичного copy проверять корень `спокойн`
- если смысл связан с безопасностью/комфортом, выражать его конкретно через ситуацию, а не через слово `спокойно`

Do not:

- не писать `спокойно обсудим`
- не писать `спокойно познакомимся`
- не писать `спокойная подготовка`
- не писать `спокойный план`
- не возвращать этот корень в лендинг, `/scenario/`, `/materials/`, pop-up, CTA, Telegram-copy или новые docs с актуальным copy
