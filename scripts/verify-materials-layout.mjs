...
const port = Number(process.env.MATERIALS_LAYOUT_PREVIEW_PORT || 4500 + (process.pid % 500));
const url = `http://127.0.0.1:${port}/materials/`;
...
try {
  ...
  await run(process.execPath, ["scripts/check-materials-layout.mjs"], {
    env: {
      ...process.env,
      MATERIALS_LAYOUT_URL: url,
    },
  });
} finally {
