...
- Предпочтительный порядок:
  ...
  2. скопировать этот паттерн;
  3. поменять только контент, ссылки, IDs и минимально необходимую геометрию;
  4. не менять шрифтовую систему, композиционный принцип кнопок, тип скруглений, hover-механику и визуальный ритм без прямого запроса.
