...
- Для общего финального CTA-портрета mobile crop — отдельный universal contract: при `<=640px` `renderExpertConversionContour` всегда ставит `object-position: 62% 18%`. Цель — не просто не обрезать голову: верх волос должен иметь небольшой симметричный визуальный зазор от верхней границы stencil (примерно `10–24px` на `430x932`), без пустого «потолка» и без среза. Не наследовать desktop/page crop на mobile; постоянные probes — `375x812`, `390x844`, `430x932`, `440x956`. CSS/DOM-check сам по себе не достаточен: перед release обязательно визуально посмотреть кадр на `430x932`.
- Универсальная visual-crop приёмка: при любом изменении `object-position`, `object-fit`, mask, aspect-ratio или photo asset нужно на точном CSS viewport посмотреть rendered кадр целиком и зафиксировать baseline/after в UI evidence. Проверяются одновременно: (1) отсутствует срез значимого объекта — лица, верхушки головы, рук или предмета; (2) композиция сбалансирована — объект не «утоплен» и не прижат к краю, нет случайного пустого поля; (3) визуальные зазоры в mask/frame симметричны и осмысленны для композиции. Computed CSS, отсутствие overflow и DOM presence не являются visual acceptance без такого просмотра.
